Feature: Associate metadata with an account
  ...
  @bvt
  Scenario: Alice wants to add another copy of her educational certificate to her account
    Given Alice had
    And Alice request Bob to notarized her certificate
    And Bob submit request to add a notarized certificate to Alice account
    When Alice approved the transaction
    Then Alice should have a copy of her certificate attached to her account.
