...
public class BlocksInfoMapper implements Function<JsonObject, BlockInfo> {
	...
	public BlockInfo apply(final JsonObject jsonObject) {
		...
		final List<String> transactionHashes = metaJsonObject.getJsonArray("transactionMerkleTree").getList();
		...
		final String signer = blockJsonObject.getString("signer");
		...
		final String blockTransactionsHash = blockJsonObject.getString("blockTransactionsHash");
		final String blockReceiptsHash = blockJsonObject.getString("blockReceiptsHash");
		final String stateHash = blockJsonObject.getString("stateHash");
		final String beneficiary = blockJsonObject.getString("beneficiary");
		return BlockInfo.create(hash, generationHash, totalFee, numTransactions, transactionHashes, signature, signer, version, type,
				height, timestamp, difficulty, feeMultiplier, previousBlockHash, blockTransactionsHash, blockReceiptsHash, stateHash,
				beneficiary);
	}
}
