...
import io.nem.sdk.infrastructure.directconnect.dataaccess.database.mongoDb.ChainInfoCollection;
...
public class BlockchainDao implements BlockchainRepository {
	...
	public Observable<BlockInfo> getBlockByHeight(final BigInteger height) {
		return Observable.fromCallable(() -> new BlocksCollection(catapultContext).find(height.longValue()).get());
	}
	...
	public Observable<List<Transaction>> getBlockTransactions(final BigInteger height) {
		return Observable.fromCallable(() -> new TransactionsCollection(catapultContext).findByBlockHeight(height.longValue()));
	}
	...
	public Observable<BigInteger> getBlockchainHeight() {
		return Observable.fromCallable(() -> new ChainInfoCollection(catapultContext).get().getNumBlocks());
	}
	...
	public Observable<BigInteger> getBlockchainScore() {
		return Observable.fromCallable(() -> {
			final ChainInfo chainInfo = new ChainInfoCollection(catapultContext).get();
			return chainInfo.getScoreHigh().shiftLeft(64 /*sizeof(long)*/).add(chainInfo.getScoreLow());
		});
	}
}
