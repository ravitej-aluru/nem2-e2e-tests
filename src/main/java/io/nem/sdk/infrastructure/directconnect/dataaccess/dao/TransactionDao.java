...
public class TransactionDao implements TransactionRepository {
	...
	public Observable<TransactionStatus> getTransactionStatus(final String transactionHash) {
		return Observable.fromCallable(() -> {
			final List<TransactionState> transactionStates =
					Arrays.asList(new UnconfirmedTransactionsCollection(catapultContext),
							new PartialTransactionsCollection(catapultContext),
							new TransactionsCollection(catapultContext),
							new TransactionStatusesCollection(catapultContext));
			for (TransactionState transactionState : transactionStates) {
				final Optional<TransactionStatus> transactionStatus = transactionState.getStatus(transactionHash);
	 ...
	 */
	@Override
	public Observable<TransactionAnnounceResponse> announce(final SignedTransaction signedTransaction) {
		return Observable.fromCallable(() -> {
			new TransactionConnection(catapultContext.getAuthenticatedSocket()).announce(signedTransaction);
	...
	public Observable<TransactionAnnounceResponse> announceAggregateBonded(SignedTransaction signedTransaction) {
		return Observable.fromCallable(() -> {
			new TransactionConnection(catapultContext.getAuthenticatedSocket()).announceAggregateBonded(signedTransaction);
			return new TransactionAnnounceResponse("Success");
		});
	}
	...
	public Observable<TransactionAnnounceResponse> announceAggregateBondedCosignature(
		...
		return Observable.fromCallable(() -> {
			new TransactionConnection(catapultContext.getAuthenticatedSocket())
