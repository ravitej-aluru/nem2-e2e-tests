...
package io.nem.automation.crosschainswap;
...
import io.nem.automationHelpers.helper.SecretLockHelper;
import io.nem.automationHelpers.helper.SecretProofHelper;
...
public class ExchangeAssetsAcrossDifferentBlockchains extends BaseTest {
	...
	public void createSecretSeed(final String userName, final LockHashAlgorithmType hashType) {
		final int NO_OF_RANDOM_BYTES = getTestContext().getScenarioContext().isContains(SECRET_HASH_LENGTH) ?
				getTestContext().getScenarioContext().getContext(SECRET_HASH_LENGTH) : CommonHelper.getRandomValueInRange(1, 1000);
		...
		final byte[] secretHashBytes = new SecretLockHelper(getTestContext()).getHashInLittleEndian(hashType, randomBytes);
	...
	public void createSecretLockForMosaic(final String userName, final BigInteger amount, final String namespaceName,
		...
		final LockHashAlgorithmType hashType = getTestContext().getScenarioContext().getContext(SECRET_HASH_TYPE);
		storeUserInfoInContext(userName);
		storeUserInfoInContext(recipientName);
		new SecretLockHelper(getTestContext()).submitSecretLockTransactionAndWait(senderAccount, mosaicToLock, numberOfBlocks, hashType
				, secretHash, resolveRecipientAddress(recipientName));
	}

	@And("^(\\w+) tries to lock (\\d+) \"(.*)\" for (.*) on the network for (-?\\d+) blocks?$")
	public void triesToCreateSecretLockForMosaic(final String userName, final BigInteger amount, final String namespaceName,
										  final String recipientName, final BigInteger numberOfBlocks) {
		final Account senderAccount = getUser(userName);
		final NamespaceId namespaceId = getNamespaceIdFromName(namespaceName);
		final Mosaic mosaicToLock = new MosaicHelper(getTestContext()).getMosaicFromNamespace(namespaceId, amount);
		final String secretHash = getTestContext().getScenarioContext().getContext(SECRET_HASH);
		final LockHashAlgorithmType hashType = getTestContext().getScenarioContext().getContext(SECRET_HASH_TYPE);
		new SecretLockHelper(getTestContext()).createSecretLockAndAnnounce(senderAccount, mosaicToLock, numberOfBlocks, hashType
				, secretHash, resolveRecipientAddress(recipientName));
	}
	...
	public void proveProofOfSecret(final String userName) {
		...
		new SecretProofHelper(getTestContext()).submitSecretProofTransactionAndWait(account, hashType, account.getAddress(), secretHash,
				proof);
	}
	...
	public void triesToProveProofOfSecret(final String userName) {
		...
		final LockHashAlgorithmType hashType = getTestContext().getScenarioContext().getContext(SECRET_HASH_TYPE);
		final String proof = getTestContext().getScenarioContext().getContext(SECRET_PROOF);
		new SecretProofHelper(getTestContext()).createSecretProofAndAnnounce(account, hashType, account.getAddress(), secretHash, proof);
	}
	...
	public void proveProofOfSecretIncorrectly(final String userName) {
		...
		final LockHashAlgorithmType hashType = getTestContext().getScenarioContext().getContext(SECRET_HASH_TYPE);
		final String proof = getTestContext().getScenarioContext().getContext(SECRET_PROOF);
		new SecretProofHelper(getTestContext()).createSecretProofAndAnnounce(account, hashType, account.getAddress(),
				secretHash.substring(1) + "F", proof);
	}
	...
	@When("^(\\w+) tries to prove knowing the secret's seed using \"(\\w+)\" as the hashing algorithm$")
	public void proveProofOfSecretWithIncorrectHashType(final String userName, final LockHashAlgorithmType hashType) {
		final Account account = getUser(userName);
		final String secretHash = getTestContext().getScenarioContext().getContext(SECRET_HASH);
		final String proof = getTestContext().getScenarioContext().getContext(SECRET_PROOF);
		new SecretProofHelper(getTestContext()).createSecretProofAndAnnounce(account, hashType, account.getAddress(), secretHash, proof);
