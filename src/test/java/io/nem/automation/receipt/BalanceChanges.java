...
public class BalanceChanges extends BaseTest {
    ...
    private BalanceChangeReceipt getBalanceChange(final BigInteger height,
        ...
        final Optional<BalanceChangeReceipt> balanceChangeReceipt =
            statement.getTransactionStatements().stream().map(s -> s.getReceipts())
                .flatMap(Collection::stream)
                .filter(receipt -> receipt.getType() == receiptType).findAny()
                .map(f -> (BalanceChangeReceipt) f);
        assertTrue(balanceChangeReceipt.isPresent(), "Balance change was not found");
        return balanceChangeReceipt.get();
    }

    private void verifyBalanceChangeReceipt(final BigInteger receiptHeight,
        final ReceiptType receiptType,
        final String namespaceName, final BigInteger actualAmount,
        ...
        final Mosaic mosaic = new MosaicHelper(getTestContext())
            .getMosaicFromNamespace(namespaceId, actualAmount);
        verifyBalanceChangeReceipt(receiptHeight, receiptType, mosaic, targetPublicKey);
    }

    private void verifyBalanceChangeReceipt(final BigInteger receiptHeight,
        final ReceiptType receiptType, final BigInteger actualAmount,
        final PublicKey targetPublicKey) {
        final Mosaic mosaic = new MosaicHelper(getTestContext())
            .getMosaicFromNamespace(NetworkCurrencyMosaic.NAMESPACEID, actualAmount);
        verifyBalanceChangeReceipt(receiptHeight, receiptType, mosaic, targetPublicKey);
    }

    private void verifyBalanceChangeReceipt(final BigInteger receiptHeight,
        final ReceiptType receiptType, final Mosaic actualMosaic,
        final PublicKey targetPublicKey) {
        final BalanceChangeReceipt balanceChangeReceipt =
            getBalanceChange(receiptHeight, receiptType);
        assertEquals(actualMosaic.getAmount().longValue(),
    ...
    public void definedValidEscrowContract() {
        final AggregateTransaction aggregateTransaction =
            getTestContext().<AggregateTransaction>findTransaction(TransactionType.AGGREGATE_BONDED)
                .get();
        getTestContext().getScenarioContext()
            .setContext(RECEIPT_TYPE, ReceiptType.LOCK_HASH_COMPLETED);
        getTestContext().getScenarioContext().setContext(RECEIPT_HEIGHT,
    ...
    public void lockExpires() {
        ...
        final BigInteger receiptHeight =
            hashLockTransaction.getTransactionInfo().get().getHeight()
        ...
        getTestContext().getScenarioContext()
            .setContext(RECEIPT_TYPE, ReceiptType.LOCK_HASH_EXPIRED);
    }
    ...
    public void checkForLockExpired() {
        final HashLockTransaction hashLockTransaction =
            getTestContext().<HashLockTransaction>findTransaction(TransactionType.LOCK).get();
        waitForBlockChainHeight(
            hashLockTransaction.getTransactionInfo().get().getHeight().longValue());
    }
    ...
    public void verifyLockHashReturnedReceipt(final String userName, final BigInteger amount) {
        ...
        final BigInteger receiptHeight = getTestContext().getScenarioContext()
            .getContext(RECEIPT_HEIGHT);
        final ReceiptType receiptType = getTestContext().getScenarioContext()
            .getContext(RECEIPT_TYPE);
        verifyBalanceChangeReceipt(receiptHeight, receiptType, amount,
    ...
    public void verifyLockHashSentReceipt(final String userName, final BigInteger amount) {
        ...
        verifyBalanceChangeReceipt(receiptHeight, ReceiptType.LOCK_HASH_CREATED, amount,
            account.getPublicAccount().getPublicKey());
    }
    ...
    public void verifyLockHashExpired(final BigInteger amount) {
        ...
        final ReceiptType receiptType = getTestContext().getScenarioContext()
            .getContext(RECEIPT_TYPE);
        verifyBalanceChangeReceipt(receiptHeight, receiptType, amount,
            harvesterPublicAccount.getPublicKey());
    }
    ...
    public void checkSecretLock() {
        final SecretLockTransaction secretLockTransaction =
            getTestContext().<SecretLockTransaction>findTransaction(TransactionType.SECRET_LOCK)
                .get();
        final BigInteger receiptHeight =
            secretLockTransaction.getTransactionInfo().get().getHeight();
        getTestContext().getScenarioContext().setContext(RECEIPT_HEIGHT, receiptHeight);
        getTestContext().getScenarioContext()
            .setContext(RECEIPT_TYPE, ReceiptType.LOCK_SECRET_CREATED);
    }

    @Then("^(\\w+) should have (\\d+) \"(.*)\" in the secret lock$")
    public void verifySecretLockCreateReceipt(final String userName, final BigInteger amount,
        ...
        final ReceiptType receiptType = getTestContext().getScenarioContext()
            .getContext(RECEIPT_TYPE);
        verifyBalanceChangeReceipt(receiptHeight, receiptType, namespaceName, amount,
            account.getPublicAccount().getPublicKey());
    }
    ...
    public void checkSecretLockExpires() {
        ...
        getTestContext().getScenarioContext().setContext(RECEIPT_HEIGHT, receiptHeight);
        getTestContext().getScenarioContext()
            .setContext(RECEIPT_TYPE, ReceiptType.LOCK_SECRET_EXPIRED);
    }

    @Then("^(\\w+) should have (\\d+) \"(.*)\" return from the secret lock$")
    public void verifySecretLockExpiredReceipt(final String userName, final BigInteger amount,
        final String namespaceName) {
        final Account account = getUser(userName);
        final BigInteger receiptHeight = getTestContext().getScenarioContext()
            .getContext(RECEIPT_HEIGHT);
    ...
    public void checkForProofAccepted(final String userName) {
        final SecretProofTransaction secretProofTransaction =
            getTestContext().<SecretProofTransaction>findTransaction(TransactionType.SECRET_PROOF)
                .get();
        final BigInteger receiptHeight =
    ...
    public void verifySecretLockCompletedReceipt(final String userName, final String recipientName,
