...
public class ArtifactExpiration extends BaseTest {
    ...
    public void verifyAssetExpiredReceipt() {
        ...
        final Statement statement = new BlockChainHelper(getTestContext())
            .getBlockReceipts(endHeight);
        final boolean expiredReceiptFound =
            statement.getTransactionStatements().stream()
                .anyMatch(t -> t.getReceipts().stream().anyMatch(r -> {
        ...
        assertTrue(expiredReceiptFound, "Did not find any artifact expiry receipt");
    }

    @Then("^she should get an estimated time for the namespace$")
    public void verifyNamespaceExpiredReceipt() {
        final BigInteger height = getTestContext().getScenarioContext()
            .getContext(CHECK_NAMESPACE_HEIGHT);
        final ReceiptType receiptType = getTestContext().getScenarioContext()
            .getContext(CHECK_NAMESPACE_TYPE);
        final NamespaceInfo namespaceInfo =
            getTestContext().getScenarioContext().getContext(NAMESPACE_INFO_KEY);
