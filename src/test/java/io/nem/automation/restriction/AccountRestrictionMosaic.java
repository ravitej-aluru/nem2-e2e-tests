...
public class AccountRestrictionMosaic extends BaseTest {
    ...
    public void theFollowingAssetsAreRegisteredAndActive(final String userName,
    ...
    public void allowsOrBlocksReceivingTransactionsContainingTheFollowingItems(
        ...
        final AccountRestrictionType accountRestrictionType = accountRestrictionHelper
            .getAccountRestrictionType(
                restrictionOperation, restrictedItemType);
        getTestContext().getLogger()
            .LogInfo("AccountRestrictionType = %s", accountRestrictionType.toString());
    ...
    public void removesAllowedOrBlockedReceivingTransactionsContainingTheFollowingItems(
        ...
        if (restrictedItemType.equals("addresses")) {
            restrictedItems.parallelStream()
                .forEach(user -> restrictedItemsList.add(getUser(user).getAddress()));
        } else if (restrictedItemType.matches("^assets?")) {
            restrictedItems.forEach(asset -> restrictedItemsList.add(resolveMosaicId(asset)));
        } else {
            restrictedItemsList.addAll(restrictedItems);
    ...
    public void unblocksGivenAsset(final String userName, final String asset) throws Throwable {
        ...
        accountRestrictionHelper.createAccountMosaicRestrictionTransactionAndWait(signerAccount,
            AccountRestrictionType.BLOCK_MOSAIC, Collections.emptyList(),
            Arrays.asList(mosaicInfo.getMosaicId()));
...
//                "assets", new ArrayList<String>(Arrays.asList(asset)));
    ...
    public void receivingAssetsShouldRemainBlocked(final String assetType) throws Throwable {
        ...
        sendAsset.triesToTransferAsset(BaseTest.AUTOMATION_USER_ALICE, BigInteger.ONE, assetType,
            recipient);
    ...
    public void triesToUnblockReceivingAssets(final String username, final String asset)
        ...
        accountRestrictionHelper.createAccountMosaicRestrictionTransactionAndAnnounce(signerAccount,
            AccountRestrictionType.BLOCK_MOSAIC, Collections.emptyList(),
            Collections.singletonList(mosaicInfo.getMosaicId()));
    }
    ...
    public void triesToBlockReceivingAssets(final String username, final String asset)
        ...
        MosaicInfo mosaicInfo = getTestContext().getScenarioContext().getContext(asset);
        accountRestrictionHelper.createAccountMosaicRestrictionTransactionAndAnnounce(signerAccount,
            AccountRestrictionType.BLOCK_MOSAIC,
            Collections.singletonList(mosaicInfo.getMosaicId()),
    ...
    public void triesToOnlyAllowReceivingAssets(final String username, final String asset)
        ...
        accountRestrictionHelper.createAccountMosaicRestrictionTransactionAndAnnounce(signerAccount,
            ...
            Collections.singletonList(mosaicInfo.getMosaicId()), Collections.emptyList());
    }
    ...
    public void triesToRemoveFromAllowedAssets(final String username, final String asset)
        ...
        MosaicInfo mosaicInfo = getTestContext().getScenarioContext().getContext(asset);
        accountRestrictionHelper.createAccountMosaicRestrictionTransactionAndAnnounce(signerAccount,
            AccountRestrictionType.ALLOW_INCOMING_MOSAIC, Collections.emptyList(),
            Collections.singletonList(mosaicInfo.getMosaicId()));
    }

    @Given("^(\\w+) has already (allowed|blocked) receiving (\\d+) different assets$")
    public void hasBlockedReceivingDifferentAssets(final String username,
        ...
        for (int i = 0; i < count; i++) {
            assets.add(RandomStringUtils.randomAlphanumeric(10));
        }
    ...
    public void userTriesToAddTooManyModificationsInATransaction(final String username,
        final int count) {
        final Account signerAccount = getUser(username);
        ...
        assets.stream().limit(count + 1).collect(Collectors.toList()).parallelStream()
            .forEach(asset -> {
                MosaicInfo mosaicInfo = getTestContext().getScenarioContext().getContext(asset);
                additions.add(mosaicInfo.getMosaicId());
            });
        accountRestrictionHelper.createAccountMosaicRestrictionTransactionAndAnnounce(signerAccount,
            AccountRestrictionType.BLOCK_MOSAIC, additions, Collections.emptyList());
    }

    @Given("^(\\w+) has (\\d+) different assets registered and active$")
    public void userHasGivenNumberOfDifferentAssetsRegisteredAndActive(final String username,
        final int count) {
        List<String> assets = new ArrayList<>(count);
        for (int i = 0; i < count; i++) {
            assets.add(RandomStringUtils.randomAlphanumeric(10));
