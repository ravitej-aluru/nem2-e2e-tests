...
public class RegisterSubNamespace extends BaseTest {
    ...
    private void verifySubNamespaceInfo(final String userName, final String namespace) {
        ...
        assertEquals(NamespaceRegistrationType.SUB_NAMESPACE, namespaceInfo.getRegistrationType(),
            errorMessage);
        assertEquals(namespaceParts.length, namespaceInfo.getDepth().intValue(), errorMessage);
        final NamespaceId parentNamespaceId = getNamespaceIdFromName(getParentName(namespace));
        assertEquals(
            parentNamespaceId.getIdAsLong(),
            namespaceInfo.parentNamespaceId().getIdAsLong(), errorMessage);
        assertTrue(namespaceInfo.isSubnamespace(), errorMessage);
    }
